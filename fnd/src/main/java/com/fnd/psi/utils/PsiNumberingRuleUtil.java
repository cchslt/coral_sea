 ...
 * @see
 */
public class PsiNumberingRuleUtil {
    ...
    /**
	 * 根据编码头部 获取  头部+ 19位 共21位 编码
	 * @param codeHeader
	 * @return
	 */
	public static String getSnowCodeByHeader(String codeHeader) {
		String rtn=codeHeader+ SnowFlakeUtil.getNextId();;
		return rtn;
	}
     ...
     * @return
     */
    public static String getStrSnowCode() {
     ...
     * @return
     */
    public static String getCodeByHeader(String codeHeader) {
        String rtn=codeHeader+ DatetimeUtil.getYear()+ DatetimeUtil.getMonthForString()+ DatetimeUtil.getDay()+NumberCode[DatetimeUtil.getHour()]+ DatetimeUtil.getSecAndmsec()+
                getRandomStr(5);
        return rtn;
    }
	 ...
	 * 生成订单编号
	 * @param userId
	 * @return 业务前缀+机器号+用户编号后3位+年份+月份编码+日期编码+小时编码+分秒+随机数
	 */
    ...
    public static String newBaseCode(String codePrefix,boolean ifNeedTime,String codeCenter,int randomLength,String codeSuffix) {
        ...
        codePrefix = StrUtil.isEmpty(codePrefix)?"":codePrefix;
        codeCenter = StrUtil.isEmpty(codeCenter)?"":codeCenter;
        codeSuffix = StrUtil.isEmpty(codeSuffix)?"":codeSuffix;
...
//        System.out.println(newCodeByHeader("P"));
        ...
        System.out.println(getCodeByHeader("PSI"));
